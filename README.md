...
- **Live weather** — temperature (bold) and conditions displayed bottom-left
...
- **Configurable** — colors, temperature unit (F/C), weather update frequency, GPS or static location, and Bluetooth vibration (powered by [Clay](https://github.com/pebble-dev/clay))
...
The configuration UI is generated by [Clay](https://github.com/pebble-dev/clay), the Pebble config framework. No external HTML pages are needed — Clay builds the config page from a TypeScript definition at `src/pkjs/clay-config.ts`.

To modify config options, edit `src/pkjs/clay-config.ts` and rebuild. The API key is persisted on the phone side by PebbleKit JS, prefilled into the Clay page on open, and stripped from the AppMessage payload before config is sent to the watch.
...
| Clay custom function | `src/pkjs/custom-clay.ts` | Prefills the API key input from PKJS-provided user data |
